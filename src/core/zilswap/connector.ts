...
import { Pool, TokenDetails, Hex} from "hashswap-zilliqa-js-sdk/lib/hex";
import { Network } from "hashswap-zilliqa-js-sdk/lib/constants";
 ...
 *
 * @member network {@link Hex.Network}
 */
export class ZilswapConnector {
  static setSDK = (sdk: Hex | null) => {
    zilswap = sdk
  }
  ...
  static approveTokenTransfer = async (props: ApproveTxProps) => {
    ...
    const observedTx = await zilswap.approveTokenTransferToHexIfRequired(props.tokenID, props.tokenAmount, props.spenderAddress);
    if (observedTx)
