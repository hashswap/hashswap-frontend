...
import { CreatePoolDialog, NewPoolMessage, PoolToggleButton, SponsorDeposit, SponsorWithdraw } from "./components";
...
const PoolView: React.FC<React.HTMLAttributes<HTMLDivElement>> = (props: any) => {
  ...
  const { showSPoolType: spoolType, showCreatePool } = layoutState;
  ...
  return (
    <MainCard {...rest} className={cls(classes.root, className)}>
	  ...
	  {/* <Button className={classes.createButton} startIcon={<PlusSVG className={classes.plusIcon}/>} onClick={() => onShowCreatePool("open")}>
          ...
          {spoolType === "addSponsor" && (<SponsorDeposit />)}
          {spoolType === "removeSponsor" && (<SponsorWithdraw />)}
