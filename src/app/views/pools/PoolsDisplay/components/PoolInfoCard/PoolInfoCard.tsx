...
import { PoolSwapVolumeMap, RootState, TokenInfo, TokenState } from "app/store/types";
    ...
    <Card {...rest} className={cls(classes.root, className)}>
      <CardContent className={classes.title}>
        <Box display="flex" alignItems="center">
          {/* <PoolLogo className={classes.poolIcon} pair={[token.symbol, "HUSD"]} tokenAddress={token.address} /> */}
        ...
        <Box display="flex" flexDirection="column" className={classes.liquidityVolumeContainer}>
          ...
          <KeyValueDisplay marginBottom={2.25} kkey="24-Hour Volume" ValueComponent="span">
            <Text align="right" className={classes.label}>
              <span className={classes.textColoured}>{swapVolumes[token.address]?.totalZilVolume.shiftedBy(-12).dp(0).toFormat()} HUSD</span> (${totalZilVolumeUSD?.dp(0).toFormat()})
            </Text>
          </KeyValueDisplay>
        </Box>
      </CardContent>
