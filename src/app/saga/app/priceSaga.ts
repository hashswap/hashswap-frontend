...
import { BIG_ONE, BIG_ZERO, PollIntervals, HUSD_ADDRESS, ZIL_DECIMALS } from "app/utils/constants";
  ...
  const prices = Object.values(tokens).reduce((accum, token) => {
    if (token.pool) {
      ...
      const rate = ZilswapConnector.getExchangeRate({
        ...
        tokenOutID: HUSD_ADDRESS,
        suppressLogs: true,
      });

      // times result by price
      const husdPrice = BIG_ONE;
      const tokPrice = husdPrice.times(rate.expectedAmount.shiftedBy(-ZIL_DECIMALS));
